...
/**
 * 
 * This implementation of Map will use Binary Search Tree (BST) to deal with collisions. This is not a recommended implementation if you
 * know your hashing algorithm does not result in any collisions or tending to zero collisions.
 * 
 * However if your Map holds a large number of objects and hashing will result in a large number of collisions then this is a better
 * approach to deal with collisions where each bucket will be represented with a BST as compared to a LinkedList. The advantage is
 * improvement in access time from O(n) to O(log n) while looking up elements from a large bucket of collided hashed keys.
 * 
 * @author prrathore
 *
 */
